...
program
  ...
  .on("--help", () => console.log("  Example:\n  maji new org.example.my-app ~/Code/my-app")).action(function(packageName, path) {
...
program
  ...
  .action(function(platform, options) {
    ...
    return runScript("run-on-device", [platform, deviceTypeArg, ...Array.from(literalArgs())], env);
  });
...
program
  ...
  .action(function(platform, options) {
    ...
    if (platform) {
      ...
      return runScript("build-app", [platform, releaseArg, ...Array.from(literalArgs())], env);
    } else {
